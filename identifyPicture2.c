...
int PteSVMTrain(SVMRet *svmret, char *listPath)
{
...
double PteSVMPredict(SVMRet *svmret, char *filePath)
{
	int data[svmret->len];
	int count;
	int deltaX;
...
redo:
		...
		fclose(dataHandle);
		getScore = PteSVMPredict(svmret, targetFilePath);
		...
		while ((dirfile = readdir(dp)) != NULL)
		{
			...
			tempScore = PteSVMPredict(svmret, targetFilePath);
