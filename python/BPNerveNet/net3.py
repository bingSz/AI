    ...
    for Iter in xrange(maxIter):
        for i in xrange(m):
            ...
            for j in xrange(hideNum):
                w0[j] -= multiply(learningRate * (O - labelMat[i].T).T * w1[:, j],  multiply(multiply(H[j], (1 - H[j])), dataMat[i]))

            b -= O - labelMat[i].T
            for j in xrange(hideNum):
                a[j] -= multiply(learningRate * sum((O - labelMat[i].T).T * w1[:, j]), multiply(H[j], (1 - H[j])))
            
            print (O - labelMat[i].T).T * (O - labelMat[i].T)
    return w0, w1, a, b
