...
int predict(decisionTree *tree, int *data)
{
  ...
  while (tree != NULL)
    {
      if (tree->condition[0] == -1)
	{
	  ret = tree->condition[1];
	  tree = temp;
	  return ret;
	}
...
int main()
{
  ...
  int t[2] = {0, 1};            /* Next is testing, the correct answer is 1 */
...
{
  int targetData[rdataX][rdataY - 1];
  int targetDataOther[rdataX][rdataY - 1];
  int sign[dataY - 1];
      ...
      for (count = 0, mainAddCount = 0; count < dataX; count++)
	{
	  ...
	  for (subCount = 0, addCount = 0; subCount < dataY; subCount++)
	      ...
	      if (subCount == divisionBy)
		{
	      ...
	      targetDataOther[mainAddCount][addCount] = data[count][subCount];
	      addCount += 1;
	    }
	  mainAddCount += 1;
	}
      createTree(mainAddCount, addCount, dataX, dataY - 1, targetDataOther, tree->right);
    }
}
